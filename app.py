...
def fmt_chart(fig, h=420):
    fig.update_layout(plot_bgcolor=BG, paper_bgcolor=BG, font=dict(size=12),
                      margin=dict(l=20, r=20, t=40, b=20), height=h,
                      legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    return fig

def insight_box(text):
    st.markdown(f"<div style='background:#f0fdfa;border-left:4px solid #0D9488;padding:10px 14px;margin:8px 0;border-radius:4px;font-size:14px'>{text}</div>", unsafe_allow_html=True)
...
with tabs[4]:
    ...
    for k in K_range:
        ...
        sil_scores.append(silhouette_score(X_scaled, km.labels_, sample_size=1000, random_state=42))
    ...
    with col1:
        ...
        fig.update_layout(title="Elbow Method — Optimal k", xaxis_title="Number of Clusters (k)",
                          yaxis_title="Inertia")
        fmt_chart(fig, 380)
        st.plotly_chart(fig, use_container_width=True)
    ...
    with col2:
        ...
        fig.update_layout(title="Silhouette Score by k", xaxis_title="Number of Clusters (k)",
    ...
    st.info(f"**Optimal k = {optimal_k}** (highest silhouette score: {sil_scores[best_k_idx]:.3f})")
    ...
    for i, c in enumerate(range(optimal_k)):
        with cols[i % len(cols)]:
            ...
            st.markdown(f"""
<div style='background:linear-gradient(135deg, #f0fdfa 0%, #e0f2f4 100%);padding:16px;border-radius:10px;border:1px solid #0D9488;margin-bottom:12px'>
<h4 style='color:#0D5C63;margin:0'>🎯 C{c}: {persona_names[c]}</h4>
...
""")
    ...
    with reg_tab:
        ...
        y_reg = df["Q13_Monthly_Spend_AED"].copy()
        ...
        for name, model in models_reg.items():
            ...
            results_reg.append({"Model": name, "R²": r2, "MAE": mae, "RMSE": rmse})
            if r2 > best_r2:
                best_r2, best_reg_name = r2, name

        reg_df = pd.DataFrame(results_reg).sort_values("R²", ascending=False)
        st.markdown(f"**Best model: {best_reg_name}** (R² = {best_r2:.4f})")
        st.dataframe(reg_df.style.format({"R²": "{:.4f}", "MAE": "{:.1f}", "RMSE": "{:.1f}"}),
                     hide_index=True, use_container_width=True)

        col1, col2 = st.columns(2)
        with col1:
            # Bar chart
            reg_melt = reg_df.melt(id_vars="Model", var_name="Metric", value_name="Score")
            # Normalise for display
            fig = make_subplots(rows=1, cols=3, subplot_titles=["R²", "MAE", "RMSE"])
        ...
        with col2:
            # Actual vs predicted
            best_reg_preds = fitted_reg[best_reg_name][1]
            fig = px.scatter(x=y_test_r, y=best_reg_preds, opacity=0.5, color_discrete_sequence=[PAL[1]],
                             labels={"x": "Actual Spend (AED)", "y": "Predicted Spend (AED)"})
            max_val = max(y_test_r.max(), best_reg_preds.max())
            fig.add_trace(go.Scatter(x=[0, max_val], y=[0, max_val], mode="lines",
                                     line=dict(dash="dash", color="grey"), name="Ideal"))
            fig.update_layout(title=f"Actual vs Predicted — {best_reg_name}")
            fmt_chart(fig, 400)
            st.plotly_chart(fig, use_container_width=True)
        ...
        fig.update_layout(title=f"Feature Coefficients — {best_reg_name}")
        fmt_chart(fig, 500)
        st.plotly_chart(fig, use_container_width=True)

        st.subheader("💼 Business Interpretation — Regression")
        st.markdown(f"""
The **{best_reg_name}** explains **{best_r2*100:.1f}%** of the variance in monthly healthcare spend.

**Top spend drivers:**
- **Visit frequency** has the largest positive coefficient — more visits = higher spend (obvious but confirmed)
...
with tabs[7]:
    ...
    with col1:
        st.markdown(f"""<div style='background:#d1fae5;padding:24px;border-radius:12px;text-align:center'>
        <h1 style='color:#0D5C63;margin:0'>{yes_pct:.1f}%</h1>
        <p style='color:#0D5C63;font-size:18px;margin:0'>Definite Adopters (Yes)</p></div>""", unsafe_allow_html=True)
    with col2:
        st.markdown(f"""<div style='background:#fef3c7;padding:24px;border-radius:12px;text-align:center'>
        <h1 style='color:#92400e;margin:0'>{maybe_pct:.1f}%</h1>
        <p style='color:#92400e;font-size:18px;margin:0'>Persuadable (Maybe)</p></div>""", unsafe_allow_html=True)
    with col3:
        st.markdown(f"""<div style='background:#fee2e2;padding:24px;border-radius:12px;text-align:center'>
        <h1 style='color:#991b1b;margin:0'>{no_pct:.1f}%</h1>
        <p style='color:#991b1b;font-size:18px;margin:0'>Not Interested (No)</p></div>""", unsafe_allow_html=True)
    ...
    for title, desc in recs:
        st.markdown(f"""
<div style='background:#f8fafc;border-left:4px solid #0D9488;padding:14px 18px;margin:10px 0;border-radius:6px'>
<b style='color:#0D5C63;font-size:15px'>{title}</b><br>
<span style='font-size:14px;color:#333'>{desc}</span></div>""", unsafe_allow_html=True)
